...
###
 ...
 # @LastEditTime: 2023-02-09 23:01:14
...
then
	...
	if [[ "$check4" != *"received"* ]] && [[ "$check4" != *"transmitted"* ]];then
		...
		echo -e "${BLUE}Your IPv4: ${local_ipv4} - ${local_isp4}(AS${local_asn4})${PLAIN}"
	...
	if [[ "$check6" != *"received"* ]] && [[ "$check6" != *"transmitted"* ]];then
		...
		echo -e "${BLUE}Your IPv6: ${local_ipv6} - ${local_isp6}(AS${local_asn6})${PLAIN}"
